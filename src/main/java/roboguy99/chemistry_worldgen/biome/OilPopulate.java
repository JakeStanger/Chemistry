...
public final class OilPopulate
{
	...
	{
		boolean doGen = TerrainGen.populate(event.chunkProvider, event.world, event.rand, event.chunkX, event.chunkZ,
				event.hasVillageGenerated, EVENT_TYPE);
	...
	public void generateOil(World world, Random rand, int chunkX, int chunkZ)
	{
		...
		if (excludedBiomes.contains(biome.biomeID)) { return; }
		...
		if (type == GenType.LARGE || type == GenType.MEDIUM)
		{
			...
			if (maxHeight >= world.getActualHeight() - 1) { return; }
				...
				for (int poolY = -radius; poolY <= radius; poolY++)
				{
					for (int poolZ = -radius; poolZ <= radius; poolZ++)
					{
						...
						if (distance <= radiusSq)
						{
							...
							world.setBlockState(pos, BuildCraftEnergy.oil.block.getDefaultState(),
	...
	private boolean isReplaceableForLake(World world, BiomeGenBase biome, int x, int y, int z)
	{
		...
		if (block == biome.fillerBlock || block == biome.topBlock) { return true; }
		
		if (!block.getMaterial().blocksMovement()) { return true; }
		
		if (block.isReplaceableOreGen(world, pos, BlockHelper.forBlock(Blocks.stone))) { return true; }
	...
	{
		return isOil(world, x + 1, y, z) || isOil(world, x - 1, y, z) || isOil(world, x, y, z + 1)
				|| isOil(world, x, y, z - 1);
	}
	...
	private void setOilColumnForLake(World world, BiomeGenBase biome, int x, int y, int z, int depth, int update)
	{
		...
		{
			if (!world.isAirBlock(new BlockPos(x, y + 2, z))) { return; }
			BlockPos pos = new BlockPos(x, y, z);
			...
			for (int d = 1; d <= depth - 1; d++)
			{
				...
				if (isReplaceableFluid(world, down) || !world.isSideSolid(down.down(), EnumFacing.UP)) { return; }
	...
	private int getTopBlock(World world, int x, int z)
	{
		...
		for (; y > 0; --y)
		{
			...
			if (block instanceof BlockStaticLiquid) { return y; }
			
			if (block instanceof BlockFluidBase) { return y; }
			
			if (block instanceof IFluidBlock) { return y; }
